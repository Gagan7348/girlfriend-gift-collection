      ...
      <var-row :gutter="10"
        ...
        <var-col :span="4"
          ...
          <var-button ref="sendMsgBtnRef"
                      ...
                      @click="sendUserMsg">Send</var-button>
        </var-col>
      </var-row>
    </div>
  </div>
...
export default {
  ...
  data () {
    return {
      ...
      // Current status while waiting for user interaction.
      status: 'systemInput',
      // Current open component.
      ...
      return new Promise((resolve) => {
        this.sendSysMsgInner(messages, inputSpeed, author).then(() => {
          if (triggerNextAction) {
            // Wait for an action before sending the next message.
